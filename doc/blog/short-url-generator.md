...
* 时间与空间复杂度
  ...
  \end{align}\\
  $$
  时间复杂度为==$O(\ln(p)\sum_{i=0}^{n-1}\frac{(i-2q)}{(q-i)})$==；
...
* 分布是否均匀：使用==卡方检验==[^2]检测随机序列分布是否在`[0, p]`内均匀分布。重复随机生成+布隆过滤器去重方法的`p-value`为0.96；递增序列+随机乱序方法的`p-value`为1.0，均满足随机分布假设。

* 分布顺序是否随机：使用==排列测试==[^3]检测随机序内部数据顺序是否随机。重复随机生成+布隆过滤器去重方法和递增序列+随机乱序方法的`p-value`都为1.0，均满足随机排列假设。
