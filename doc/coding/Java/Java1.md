...
- ```java
    public class Main {
        public static void main(String[] args) {
            Pair<MyNumber> numPair = new Pair<>(new MyNumber(0));
            Pair<MyInt> intPair = new Pair<>(new MyInt(0));
            func0(numPair);
            func0(intPair);
            func1(intPair);
            func1(numPair);
        ...
        static void func0(Pair<? extends MyNumber> p) {
            // p中数据的实际类型可能MyNumber，也可能是MyNumber的子类。
            // Pair 的get方法签名由 T get(){}变成 <? extends MyNumber> get(){}
            // 即get返回MyNumber及其子类的对象，这里就可以使用父类MyNumber接收，这是数据的向上转型，是安全的，允许读操作。
            MyNumber myNumber = p.get();
    
            // Pair 的set方法签名由 void  set(T t){}变成 void set(<? extends MyNumber> t){}
            // Pair内部的数据类型是MyNumber及其子类，此时有泛型擦除Pair<? extends MyNumber> p变成Pair<MyNumber> p，
            // 只能向set 传入MyNumber对象，将MyNumber对象传入set方法对内部对象赋值，就是将父类对象赋值给子类引用，是向下转型是不安全的，
            // 其实即使P的实际类型就是MyNumber也是错误的，所以禁止写操作。
            p.set(new MyNumber(2)); //错误
            // 唯一的例外是可以传入null，因为一切对象都可以赋值为null。
            p.set(null);
    
        }
    
        static void func1(Pair<? super MyInt> p) {
            // p中数据的实际类型可能MyInt，也可能是MyInt的父类。
            // Pair 的get方法签名由 T get(){}变成 <? super MyInt> get(){}
            // 即get返回MyInt及其父类的对象，如果这里使用MyInt接收，这是数据的向下转型，是不安全的，禁止读取操作，
            // 其实即使P的实际类型就是MyInt也是错误的。
            MyInt num = p.get(); //错误
            // 唯一的例外就是可以使用Object接受返回值，因为Object是一切对象的父类，属于向上转型是允许的。
            Object o = p.get();
            // 其实可以只读取值，不把读取值进行赋值，这就不涉及向下转型的问题，可以正常运行。
            System.out.println(p.get());
    
            // Pair 的set方法签名由 void  set(T t){}变成 void set(<? super MyInteger> t){}
            // Pair内部的数据类型是MyInteger及其父类等，将MyInteger对象传入set方法对内部对象赋值，就是将子类对象赋值给父类引用，
            // 是向上转型，是安全的，允许写操作。
            p.set(new MyInt(1));
        }
...
1：Java的`java.util`包主要提供了以下三种类型的集合：
...
<img src="https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/24d8ac8a958d4de19f694068801f51f1~tplv-k3u1fbpfcp-zoom-1.image" alt="img" style="zoom:67%;" />
